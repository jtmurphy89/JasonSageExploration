...
def better_complex_to_rgb(z_values):
    """
    INPUT:
...
class BetterComplexPlot(GraphicPrimitive):
    """
...
def better_complex_plot(f, xrange, yrange, **options):
    r"""
    ...
    sig_on()
    z_values = np.array([[  f(new_CDF_element(x, y)) for x in np.arange(ranges[0][0], ranges[0][1], ranges[0][2])]
                                            for y in np.arange(ranges[0][0], ranges[0][1], ranges[0][2])])
    z_values = [z_values.tolist()]
    sig_off()
    g = Graphics()
    g._set_extra_kwds(Graphics._extract_kwds_for_show(options, ignore=['xmin', 'xmax']))
    g.add_primitive(BetterComplexPlot(better_complex_to_rgb(z_values), xrange, yrange, options))
    return g